...
namespace aa {
	...
	template<class T>
	concept stream_like = not_ref_derived_from<T, apply_traits_t<std::basic_ios, T>>;



	// Kintamojo tipas T&, o ne T&&, nes standarte naudojamas toks tipas operator<< užklojimuose.
	template<class U, class T>
	concept insertable_into = not_ref_derived_from<T, apply_traits_t<std::basic_ostream, T>>
		&& requires(apply_traits_t<std::basic_ostream, T> &t, const U & u)
		/**/ { { t << u } -> std::same_as<apply_traits_t<std::basic_ostream, T> &>; };

	template<class U, class T>
	concept extractable_from = not_ref_derived_from<T, apply_traits_t<std::basic_istream, T>>
	...
	concept output_stream = not_ref_derived_from<T, apply_traits_t<std::basic_ostream, T>> && (... && insertable_into<A, T>);

	template<class T, class... A>
	concept input_stream = not_ref_derived_from<T, apply_traits_t<std::basic_istream, T>> && (... && extractable_from<A, T>);
