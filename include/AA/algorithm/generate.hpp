...
namespace aa {
	...
	// [0, MAX - MIN + 1)
	template<class T = void, random_bit_generator G>
	...
	// [0, 1)
	template<std::floating_point T = double, random_bit_generator G>
	...
	// [0, mag)
	template<void_or_convertible_from_floating_point U = void, std::floating_point T, random_bit_generator G>
		requires (std::numeric_limits<T>::digits > std::bit_width(G::max() - G::min()))
