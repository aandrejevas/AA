...
namespace aa {
	...
	template<string_perfect_hash H, class EVAL = evaluator, tuple_evaluable_by<EVAL &, std::tuple_size_v<const_t<H>>> TUPLE>
	constexpr void parse(TUPLE &t, std::streambuf &file, EVAL &&eval = default_value) {
	...
	constexpr void safe_parse(TUPLE &t, std::streambuf &file, EVAL &&eval = default_value) {
	...
	template<string_perfect_hash H, class TUPLE, evaluator_for_tuple<TUPLE, std::tuple_size_v<const_t<H>>> EVAL = evaluator>
	constexpr std::remove_cvref_t<TUPLE> parse(std::streambuf &file, EVAL &&eval = default_value) {
	...
	template<string_perfect_hash H, class TUPLE, evaluator_for_tuple<TUPLE, std::tuple_size_v<const_t<H>>> EVAL = evaluator>
	constexpr std::remove_cvref_t<TUPLE> safe_parse(std::streambuf &file, EVAL &&eval = default_value) {
		return make_with_invocable([&](std::remove_cvref_t<TUPLE> &t) -> void {
