...
namespace aa {
	...
	struct evaluator<T> {
		...
		static constexpr void operator()(T &t, const U &token) {
	...
	struct evaluator<std::string> {
		template<size_t = 0, char_range U>
		static constexpr void operator()(std::string &t, const U &token) {
	...
	constexpr std::remove_cvref_t<T> evaluate(const U &token, EVAL &&eval = default_value) {
